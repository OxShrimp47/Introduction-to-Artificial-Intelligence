 ...
 *Last Edit:Nov 15 2019
 *Program Name: Backwards Chaining
 */
...
public class BackwardChain {
	...
	public BindingList unify(Literal lit1, Literal lit2, BindingList bl) {
		if(lit1.equals(lit2)==true) {
			BindingList temp=new BindingList();
			Binding t=new Binding();
			for (int i=0; i<lit1.args.size(); i++) {
				t.var=lit1.args.get(i).v;
				t.val=lit1.args.get(i);
			temp.pairs.add(t);
			}
			bl.compose(temp);
			for (int i=0; i<lit2.args.size(); i++) {
				t.var=lit2.args.get(i).v;
				t.val=lit2.args.get(i);
			temp.pairs.add(t);
			}
			bl.compose(temp);
			return bl;
	...
	public BindingList unify(Function f1, Function f2, BindingList bl) {
		if(f1.equals(f2)==true) {
			...
			for (int i=0; i<f1.args.size(); i++) {
				t.var=f1.args.get(i).v;
				t.val=f1.args.get(i);
			temp.pairs.add(t);
			}
			bl.compose(temp);
			for (int i=0; i<f2.args.size(); i++) {
				t.var=f2.args.get(i).v;
				t.val=f2.args.get(i);
			temp.pairs.add(t);
			}
			bl.compose(temp);
