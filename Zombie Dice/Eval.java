 ...
 *Last Edit:Dec 13 2019
 *Program Name: Zombie Dice
 */
...
public class Eval {
		...
		for(DieFace x : DieFace.values() ) {
			...
			for(DieFace y : DieFace.values() ) {
				...
				for(DieFace z : DieFace.values() ) {
					dice3.up=z;
					if(dice1.up!=DieFace.invalid&&dice2.up!=DieFace.invalid&&dice3.up!=DieFace.invalid) {
						double outcome = s.rollProb(dice1.up,dice2.up,dice3.up);
						s.roll(dice1.up,dice2.up,dice3.up);
						val += outcome * value_rolled_hand(s,depth);
					}		
				}	
			}
		}
	...
	static public double heuristic(State s) {
		...
		double Total = (double) (s.comp_brains_eaten + s.user_brains_eaten + s.brains_collected);
		if(s.current_player==Turn.computer) {
			HypoVal= Total- (double)(s.user_brains_eaten);

		}
		else {
			HypoVal= (double) (s.comp_brains_eaten);
		}
		
		double diff = HypoVal - (double)(s.user_brains_eaten);
		diff=diff/Total;
		double gap= diff/2 +0.5;
		double gauge = (13-(double)(State.brains_to_win))/13;

		double WinRate = (gauge + gap)/2;
		

		value = Math.min(WinRate, State.win_payoff);
		value = Math.max(WinRate, -1 * State.win_payoff);
